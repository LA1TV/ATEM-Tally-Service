...
atem.on('stateChanged', function(err, state) {
  ...
  if (state.tallys[1] == 1) { //Camera 1
    red1.write(1, function(err) {
      if (err) throw err;
    });
  } else {
    red1.write(0, function(err) {
      if (err) throw err;
    });
  }

  if (state.tallys[2] == 1) { //Camera 2
    red2.write(1, function(err) {
      if (err) throw err;
    });
  } else {
    red2.write(0, function(err) {
      if (err) throw err;
    });
  }
  if (state.tallys[3] == 1) { //Camera 3
    red3.write(1, function(err) {
      if (err) throw err;
    });
  } else {
    red3.write(0, function(err) {
      if (err) throw err;
    });
  }
